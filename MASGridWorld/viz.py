...
class Viz(object):
    ...
    def single_frame(self, environment):
        ...
        # row_pixels = cell_size * n_rows + (n_rows + 1) * cell_border
        # col_pixels = cell_size * n_cols + (n_cols + 1) * cell_border
        ...
        for r in range(n_rows):
            for c in range(n_cols):
                top_left = (c * cell_width + (c + 1) * borders_between_cols,
                            r * cell_height + (r + 1) * borders_between_rows)
                bot_right = ((c + 1) * (cell_width + borders_between_cols),
                             (r + 1) * (cell_height + borders_between_rows))
                cv2.rectangle(
                    img,
                    top_left,
                    bot_right,
